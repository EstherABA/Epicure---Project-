    ...
    const chefs = data.chefs.map((chef) => { 
        return ( 
          ...
          <div className='chefs-data-container'>
            <ChefPortraitCard 
            src={require(`../../${chef.portrait}`)} alt={`${chef.name}`} name={`${chef.name}`} description={`${chef.description}`} />
            <p className=' title-of-the-week'>{`${chef.name} Restaurants:` } </p>
                <div className='chef-restaurants'>
                    { 
                    chef.restaurants.map((res)=> <HomeCard ImgSrc={require(`../../${data.restaurants[3].image}`)} ImgAlt={res} name={res} />)
                    }
                </div>
            </div>) :
            (<></>)
